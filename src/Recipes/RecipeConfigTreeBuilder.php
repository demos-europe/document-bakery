            ...
            ->arrayPrototype()
                ...
                ->children()
                    ...
                    ->arrayNode('styles')
                        ->arrayPrototype()
                            ->performNoDeepMerging()
                            ->info('There can be multiple styles. These are the recipe styles.')
                            ->children()
    ...
    public function addInstructionsArrayNode($name, $depth = 0): NodeDefinition
    {
        ...
        $node->arrayPrototype()
            ->children()
                ...
                ->end()
                ->arrayNode('style')
                    ->info('This is the local style definition. Options need to be under attributes.')
                    ->children()
                        ->scalarNode('name')
                            ->info('The name of the used style (must be defined either globally or in the styles section of the recipe)')
                        ->end()
                        ->arrayNode('attributes')
                            ->scalarPrototype()
                            ->end()
